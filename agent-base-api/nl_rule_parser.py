...
def _build_image_prompt(
    ...
    """Görsel üretim prompt'u — şablona SADIK kalma odaklı.

    Temel kural: Bu bir yeni görsel üretimi DEĞİL, şablon adaptasyonudur.
    Şablonun layout'u, renk paleti, tipografi, buton stili ve kompozisyonu
    AYNEN korunmalı; sadece ürün görseli ve metin içerikleri değiştirilmeli.
    """
    name = (
    ...
    # Şablon verisi varsa oradaki prompt'u al — ama şablona sadakat talimatını MUTLAKA ekle
    base_prompt: str | None = None
    if template_data:
        for k in ("imagePrompt", "image_prompt", "imageDescription", "visualPrompt", "prompt"):
            val = template_data.get(k)
            if isinstance(val, str) and val.strip():
                base_prompt = val.strip()
                break

    if not base_prompt:
        base_prompt = (
            f"Sosyal medya görseli: {name}, tema {template_name}, {channel} formatı. "
            f"GÖRSELDEKİ TÜM YAZI VE METİNLER TÜRKÇE OLMALI. İngilizce metin kullanma."
        )

    # --- ŞABLON SADAKAT BLOĞU (her zaman eklenir) ---
    # Bu blok AI'nın şablonu görmezden gelmesini engeller.
    fidelity_instruction = (
        "\n\n=== TEMPLATE ADAPTATION — NOT A NEW DESIGN ==="
        "\nThis is STRICTLY a template adaptation task. You MUST:"
        "\n1. PRESERVE the EXACT layout structure of the reference template image"
        "\n2. PRESERVE the EXACT color palette (background color, accent colors, gradients)"
        "\n3. PRESERVE the EXACT typography style (font weight, size hierarchy, text placement zones)"
        "\n4. PRESERVE all UI elements: buttons, badges, price tags, borders, icons — same style and position"
        "\n5. PRESERVE the overall composition and visual hierarchy"
        "\n6. ONLY replace: the product photo and text content (name, price, description)"
        "\nDO NOT: invent new layouts, change color schemes, add new design elements, or create a fresh design."
        "\nThe output must look like the SAME template with different product content inserted."
        "\n=== END TEMPLATE INSTRUCTIONS ==="
    )
    return base_prompt + fidelity_instruction
...
def _generate_image_via_pipeline(
    *,
    ...
    if reference_image_url or reference_image_urls:
        extras_text.append(
            "CRITICAL: The reference template image is the MASTER DESIGN. "
            "Copy its layout EXACTLY — do not deviate from the template structure, "
            "color palette, typography, button placement, or any visual element. "
            "This is template ADAPTATION not creative generation. "
            "Treat the template as a locked design frame."
        )
    if product_image_url or product_image_urls:
        extras_text.append(
            "The product image provided is the ONLY element that should change — "
            "place it in the product zone of the template, do not alter anything else."
        )
    if store_logo_url:
        extras_text.append(
            "Place the store logo exactly where the template's logo zone is, same size and style."
        )
    if extras_text:
    ...
    try:
        ...
        # template_image_weight: şablon referansının ağırlığı.
        # 0.9 → AI şablonu %90 oranında takip eder, %10 içerik adaptasyonu yapar.
        # Şablon sadakati sorununda bu değeri 0.85-0.95 arasında tutun.
        _has_template_ref = bool(all_refs)
        images = _sync_generate_images_task(
            ...
            # Şablon referans ağırlığı — yüksek değer = şablona daha sadık
            image_weight=0.9 if _has_template_ref else None,
            strength=0.35 if _has_template_ref else None,  # düşük strength = şablona sadık
        )
